...
public class FlightBookingServiceImpl implements FlightBookingService {
    ...
    public void cancelFlight(Long flightId, String passengerEmail) {
        ...
        if (flight.getPassengerEmail() == null
                || !flight.getPassengerEmail().equals(passengerEmail)) {
    ...
    public List<AvailableFlightDTO> findAvailableFlights() {
        return flightBookingRepository.findByStatus(FlightStatus.AVAILABLE).stream()
                .map(mapper::toAvailableFlightDTO).collect(Collectors.toList());
    }
    ...
    public List<FlightListDTO> findAll() {
        return flightBookingRepository.findAll().stream().map(mapper::toListDTO)
