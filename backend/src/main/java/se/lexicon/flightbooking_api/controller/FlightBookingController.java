...
public class FlightBookingController {
    ...
    @ApiResponses(
            value = {@ApiResponse(responseCode = "200", description = "Flight successfully booked"),
                    @ApiResponse(responseCode = "404", description = "Flight not found",
                            content = @Content),
                    @ApiResponse(responseCode = "400", description = "Flight not available",
                            content = @Content)})
    ...
    public ResponseEntity<FlightBookingDTO> bookFlight(
            ...
            @Parameter(
                    description = "Booking request details") @Valid @RequestBody BookFlightRequestDTO bookingRequest) {
        return ResponseEntity.ok(flightBookingService.bookFlight(flightId, bookingRequest));
    }

    @Operation(summary = "Get bookings by email",
            description = "Returns all bookings for a given email")
    ...
    @Operation(summary = "Cancel a flight booking",
            description = "Cancels a flight booking for a given flight ID and email")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "204", description = "Flight successfully cancelled"),
            @ApiResponse(responseCode = "404", description = "Flight not found",
                    content = @Content),
            @ApiResponse(responseCode = "400", description = "Invalid email for the booking",
                    content = @Content)})
    ...
    public ResponseEntity<Void> cancelFlight(
            ...
            @Parameter(
                    description = "Email associated with the booking") @RequestParam String email) {
