...
class FlightBookingControllerTest {
    ...
    void getAllFlightsReturnsEveryFlight() throws Exception {
        flightBookingRepository
                .save(createFlight("FL001", FlightStatus.AVAILABLE, null, null, "London"));
        flightBookingRepository.save(createFlight("FL002", FlightStatus.BOOKED, "Jane Doe",
                "jane@example.com", "Paris"));
    ...
    void getAvailableFlightsReturnsOnlyAvailableFlights() throws Exception {
        ...
        mockMvc.perform(get("/api/flights/available")).andExpect(status().isOk())
    ...
    void bookFlightBooksAnAvailableFlight() throws Exception {
        FlightBooking availableFlight = flightBookingRepository
                .save(createFlight("FL001", FlightStatus.AVAILABLE, null, null, "London"));
        ...
        FlightBooking savedFlight =
                flightBookingRepository.findById(availableFlight.getId()).orElseThrow();
    ...
    void getBookingsByEmailReturnsOnlyBookedFlightsForEmail() throws Exception {
        flightBookingRepository.save(createFlight("FL001", FlightStatus.BOOKED, "Jane Doe",
                "jane@example.com", "London"));
        flightBookingRepository.save(
                createFlight("FL002", FlightStatus.AVAILABLE, null, "jane@example.com", "Paris"));
        flightBookingRepository.save(
                createFlight("FL003", FlightStatus.BOOKED, "John Doe", "john@example.com", "Rome"));
    ...
    void cancelFlightMakesFlightAvailableAndClearsPassengerData() throws Exception {
        FlightBooking bookedFlight = flightBookingRepository.save(createFlight("FL001",
                FlightStatus.BOOKED, "Jane Doe", "jane@example.com", "London"));

        mockMvc.perform(delete("/api/flights/{flightId}/cancel", bookedFlight.getId())
                .param("email", "jane@example.com")).andExpect(status().isNoContent());

        FlightBooking savedFlight =
                flightBookingRepository.findById(bookedFlight.getId()).orElseThrow();
    ...
    void cancelFlightReturnsBadRequestWhenFlightIsNotBooked() throws Exception {
        ...
        mockMvc.perform(delete("/api/flights/{flightId}/cancel", availableFlight.getId())
                .param("email", "jane@example.com")).andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.detail").value("Flight is not currently booked"));
    }

    private FlightBooking createFlight(String flightNumber, FlightStatus status,
            String passengerName, String passengerEmail, String destination) {
        LocalDateTime departureTime = LocalDateTime.of(2026, 5, 1, 10, 0);
        return FlightBooking.builder().flightNumber(flightNumber).passengerName(passengerName)
                .passengerEmail(passengerEmail).departureTime(departureTime)
                .arrivalTime(departureTime.plusHours(2)).status(status).destination(destination)
                .price(199.99).build();
    }
}
